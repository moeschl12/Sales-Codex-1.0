...
| Research Program Integration (ein einzelnes W-XXX-Projekt abgeschlossen, Stufe 8/9 bestanden) *(präzisiert 2026-07-06, V11-03)* | Compiler-Baseline-Check **verpflichtend** (ohnehin Teil des Health-Check-Prüfpunkts „Atlas-Kompatibilität" in Stufe 9) | Eine einzelne Repository Integration fügt typischerweise wenige neue Knoten/Kanten hinzu (z. B. ein neues MEC plus Cross-Links) — strukturell vergleichbar mit „Einzelnes Buch abgeschlossen", nicht mit einem vollen Content-Analysis-Trigger. |
| Mehrere Research Program Integrations kumuliert seit dem letzten Zyklus (Richtwert, kein hartes Kriterium: ab drei abgeschlossenen W-Projekten) *(präzisiert 2026-07-06, V11-03)* | Gezielter Structure-×-Evidence-Review empfohlen, keine volle Content Analysis zwingend | Analog zum bereits bestehenden Buch-Batch-Grundsatz (Zeile oben, Grundsatz-Fußnote): Einzeleffekte sind zu inkrementell, aber die kumulative Wirkung mehrerer Integrationen (hier: W-002, W-003, W-004 zwischen 2026-07-05 und 2026-07-06) kann Hub-/Cluster-Verschiebungen erzeugen, die ein einzelner Baseline-Check nicht abbildet. |
...
| Zyklus 2 — Compiler-Baseline-Check (V11-03) | 2026-07-06 | Kumulierte Research Program Integration (W-002, W-003, W-004 abgeschlossen seit Sprint 1) + V1.1-Kontrollplan-Konsolidierung (V11-01/V11-02); geprüft gegen die neu präzisierte Trigger-Zeile oben — kein Kriterium für volle Content Analysis erfüllt (keine RC, kein Major Release, keine Framework-Änderung, kein gebündelter Hoch-Priorität-Neuzugang im Sinne von Zeile „Scientific-Debt-Update gebündelt") | 0 | 18 (Δ 0 — unverändert ggü. Sprint 1) | 1 (unverändert — kein Hinweis auf eine zweite, verborgene Parallelwissensbasis) | nicht separat beziffert (kein Trigger für volle Content Analysis erfüllt, siehe Abschnitt 3 — Structure × Evidence Review ist nur deren verpflichtender Bestandteil) | nicht separat beziffert (dito) |

**Methodische Notiz zu Zyklus 2:** Compiler-Lauf (515 Nodes, 2111 Edges, 2 unaufgelöste Referenzen auf `T-0048`, 5 Genitiv-Normalisierungen, 0 Fence-Balance-Warnungen — `08_knowledge_atlas/output/atlas_compiler_report.md`) dreifach reproduziert, Determinismus bestätigt (siehe `00_project/projects/V11-01_baseline_control_plane/COMPLETION_REPORT.md`, Abschnitt 4.2). Connected-Components-Wert (Spalte 6) wurde für diesen Zyklus erstmals seit Sprint 1 erneut unabhängig berechnet (Union-Find über `edges.csv`, beschränkt auf Knoten vom Typ A/MEC/P/T/MOD, ST ausgeschlossen — 206 Knoten, 1067 interne Kanten, 1 Komponente) — nicht nur aus Sprint 1 übernommen.
